...
HistoricalPowerTrack.prototype.createJob = function(config, cb) {
	...
	var json = {
		rules: config.rules,
		fromDate: config.fromDate ? new Date(config.fromDate.toUTCString()).toFormat('YYYYMMDDHH24MI') : null,
		toDate: config.toDate ? new Date(config.toDate.toUTCString()).toFormat('YYYYMMDDHH24MI') : null,
		title: config.title,
